...
**Arch Linux** 以其极高的自定义性和简易的包管理吸引了不少用户，但是这也带来了一个问题，就是不太友好的安装过程。虽然可以用官方的 `archinstall` 但是这很容易让初学者之后面对一些基本问题束手无策。想自己安装？按照官方的教程装完了，开机就一个大黑屏终端，不知道干什么。所以这篇笔记就诞生了，这篇笔记主要在官方安装教程的基础上添加了关于显卡驱动，桌面环境等等这些开箱即用的配置，也欢迎各位大佬指出问题。
